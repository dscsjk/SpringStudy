package com.doxmo.web.dto;

public class Dxm07 {
	
	private String order_no ;
	private String u_id	;
	private String store_cd	;
	private int tt_pay ;
	private String pay_tp ;
	private String rcpt_tp ;
	private String rcpt_nm ;
	private String rcpt_tel	;
	private String rcpt_addr ;
	private String memo	;
	private String order_st	;
	private String upd_dt ;
	private String crt_dt ;
	
	public String getOrder_no() {
		return order_no;
	}
	public void setOrder_no(String order_no) {
		this.order_no = order_no;
	}
	public String getU_id() {
		return u_id;
	}
	public void setU_id(String u_id) {
		this.u_id = u_id;
	}
	public String getStore_cd() {
		return store_cd;
	}
	public void setStore_cd(String store_cd) {
		this.store_cd = store_cd;
	}
	public int getTt_pay() {
		return tt_pay;
	}
	public void setTt_pay(int tt_pay) {
		this.tt_pay = tt_pay;
	}
	public String getPay_tp() {
		return pay_tp;
	}
	public void setPay_tp(String pay_tp) {
		this.pay_tp = pay_tp;
	}
	public String getRcpt_tp() {
		return rcpt_tp;
	}
	public void setRcpt_tp(String rcpt_tp) {
		this.rcpt_tp = rcpt_tp;
	}
	public String getRcpt_nm() {
		return rcpt_nm;
	}
	public void setRcpt_nm(String rcpt_nm) {
		this.rcpt_nm = rcpt_nm;
	}
	public String getRcpt_tel() {
		return rcpt_tel;
	}
	public void setRcpt_tel(String rcpt_tel) {
		this.rcpt_tel = rcpt_tel;
	}
	public String getRcpt_addr() {
		return rcpt_addr;
	}
	public void setRcpt_addr(String rcpt_addr) {
		this.rcpt_addr = rcpt_addr;
	}
	public String getMemo() {
		return memo;
	}
	public void setMemo(String memo) {
		this.memo = memo;
	}
	public String getOrder_st() {
		return order_st;
	}
	public void setOrder_st(String order_st) {
		this.order_st = order_st;
	}
	public String getUpd_dt() {
		return upd_dt;
	}
	public void setUpd_dt(String upd_dt) {
		this.upd_dt = upd_dt;
	}
	public String getCrt_dt() {
		return crt_dt;
	}
	public void setCrt_dt(String crt_dt) {
		this.crt_dt = crt_dt;
	}

}
